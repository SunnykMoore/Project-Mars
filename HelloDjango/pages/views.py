...
def home(request, *args, **kwargs):
	...
	deny_reorders = Order.objects.order_by('-num_reorders').exclude(status = "DENIED").exclude(hospital = "Enter Hospital")[0:4]               
	featured_prods = Product.objects.order_by('-num_orders').exclude(name = "New Product")[0:4]
	return render(request, "home.html", {'latest_orders': latest_orders, 'most_reorders': most_reorders, 'featured_prods': featured_prods, 'deny_reorders': deny_reorders})
